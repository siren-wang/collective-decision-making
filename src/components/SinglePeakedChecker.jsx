...
// Single-peaked recognition algorithm
const recognizeSinglePeaked = (profile) => {
  // Utility functions
  ...
  if (profile.length === 0) return { isSinglePeaked: false };
  const A = profile[0];
  ...
  while (A_prime.size > 0 && right.length === 0) {
    ...
    if (B.length > 2) return { isSinglePeaked: false };
  ...
  while (A_prime.size >= 2) {
    ...
    if (L.size > 1 || R.size > 1) return { isSinglePeaked: false };
    if ([...L].some((x) => R.has(x))) return { isSinglePeaked: false };

    const undecided = B.filter((x) => !L.has(x) && !R.has(x));
    for (const x of undecided) {
      if (L.size === 0) L.add(x);
      else if (R.size === 0) R.add(x);
      else return { isSinglePeaked: false };
    }
  ...
  const ordering = [...left, ...A_prime, ...right];
  return { isSinglePeaked: true, ordering };
};

const SinglePeakedChecker = () => {
  ...
  const handleCheck = () => {
    try {
      ...
      setOutput({ isSinglePeaked: false, error: "Invalid input" });
    }
  };
  ...
  return (
    <div className="single-peaked-checker">
              ...
              <div className="result-panel">
                ...
                {!output.isSinglePeaked ? (
                  <p className="result-failure">
                    The preference profile is NOT single-peaked.
...
export default SinglePeakedChecker;